// --- js/views/plantDetail.js ---
import { AppState } from '../app.js';
import { renderPlantTags } from './herbarium.js';
import { YouTubeHandler } from '../components/youtubeService.js';
import { CustomVideoHandler } from '../components/customVideo.js';
import { CustomAudioHandler } from '../components/customAudio.js';
...
const localVideoHandler = new CustomVideoHandler();
const localAudioHandler = new CustomAudioHandler();
...
export async function initDetailMap(plant) {
    ...
    if (coords.length === 0) {
        ...
        coords.forEach(coord => {
            ...
            // CORRECCIÓN: Ajustado icono nativo a material-icons
    ...
    const localVideoUrl = videoIdProp && !videoIdProp.includes('youtube.com') && !videoIdProp.includes('youtu.be') ? videoIdProp : null;
    ...
    const youtubeUrl = videoObj?.embedUrl || (videoIdProp && (videoIdProp.includes('youtube.com') || videoIdProp.includes('youtu.be')) ? videoIdProp : null);
...
function renderIslandPresence(illaString = "") {
    ...
    return illes.map(illa => {
        const present = illaString.toLowerCase().includes(illa.key.toLowerCase()) || illaString.toLowerCase().includes('totes');
        ...
        // CORRECCIÓN: Clases corregidas a material-icons nativos
        const icon = present
            ? '<span class="material-icons text-primary text-xl">check_circle</span>'
            : '<span class="material-icons text-forest-neutral-600 text-xl">cancel</span>';
...
function renderConservationCard(status = "") {
    let colorClasses = { cardBg: 'bg-emerald-500/10', cardBorder: 'border-emerald-500/20', textLight: 'text-emerald-400', progressBg: 'bg-emerald-600', textDark: 'text-emerald-600' };
    let label = 'Segura', percent = 20, desc = 'Poblacions estables.';

    if (status.includes('Crític')) {
        colorClasses = { cardBg: 'bg-red-500/10', cardBorder: 'border-red-500/20', textLight: 'text-red-400', progressBg: 'bg-red-600', textDark: 'text-red-600' };
        ...
        colorClasses = { cardBg: 'bg-amber-500/10', cardBorder: 'border-amber-500/20', textLight: 'text-amber-400', progressBg: 'bg-amber-600', textDark: 'text-amber-600' };
        label = 'Vulnerable'; percent = 65; desc = 'Vulnerable a l\'extinció si no es prenen mesures.';
    } else if (status.includes('Protegida')) {
        colorClasses = { cardBg: 'bg-orange-500/10', cardBorder: 'border-orange-500/20', textLight: 'text-orange-400', progressBg: 'bg-orange-600', textDark: 'text-orange-600' };
...
function renderRelatedCards(currentId) {
    ...
    const cards = others.map(entry => {
        ...
        const thumbUrl = imageUrl ? imageUrl.replace('_2000.webp', '_400_thumb.webp') : 'img/fallback.webp';
        ...
        <div onclick="window.navigateSPA('plant-detail', '${pid}')"
            ...
            <div class="relative flex-1 overflow-hidden">
                <img class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700"
                    alt="Fotografia de ${p.alternateName || p.name}" src="${thumbUrl}" loading="lazy" />
    ...
    <div class="mt-20 border-t border-white/10 pt-12">
        <div class="mb-8">
            <h3 class="text-3xl font-black text-white mb-2 tracking-tight">Explora més a l'Herbari</h3>
...
export function renderPlantDetail(plant) {
    ...
    const carouselSlidesHTML = images.map((imgObj, index) => {
        ...
        let altText = imgObj.caption || imgObj.description || `Fotografia de ${plant.alternateName || plant.name}`;
    ...
    const fullscreenSlidesHTML = images.map((imgObj, index) => {
        let originalUrl = imgObj.contentUrl || '';
        let altText = imgObj.caption || imgObj.description || `Fotografia a pantalla completa de ${plant.alternateName || plant.name}`;
    ...
    // CORRECCIÓN ACCESIBILIDAD: Añadidos aria-label descriptivos a los botones de navegación del carrusel
    ...
    <div id="fullscreen-gallery" class="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md opacity-0 pointer-events-none transition-opacity duration-300" onclick="window.closeFullscreenGallery()">
        ...
        <div class="absolute inset-y-0 left-0 flex items-center px-4 md:px-8 z-20 pointer-events-none">
            <button onclick="window.prevSlide(${images.length}); event.stopPropagation();" class="size-14 rounded-full bg-white/10 text-white flex items-center justify-center hover:bg-white hover:text-black backdrop-blur-md transition-all cursor-pointer pointer-events-auto" aria-label="Imatge anterior">
                <span class="material-icons text-2xl">chevron_left</span>
            </button>
        </div>
        <div class="absolute inset-y-0 right-0 flex items-center px-4 md:px-8 z-20 pointer-events-none">
            <button onclick="window.nextSlide(${images.length}); event.stopPropagation();" class="size-14 rounded-full bg-white/10 text-white flex items-center justify-center hover:bg-white hover:text-black backdrop-blur-md transition-all cursor-pointer pointer-events-auto" aria-label="Imatge següent">
                <span class="material-icons text-2xl">chevron_right</span>
            </button>
        </div>
        ` : ''}
        
        <button onclick="window.closeFullscreenGallery(); event.stopPropagation();" class="absolute top-6 right-6 size-12 rounded-full bg-white/10 text-white flex items-center justify-center hover:bg-white hover:text-black backdrop-blur-md transition-all cursor-pointer z-30" aria-label="Tancar galeria a pantalla completa">
    ...
    const carouselDotsHTML = images.length > 1 ? images.map((_, index) => `
        <button onclick="window.setSlide(${index})" id="carousel-dot-${index}" class="size-2 rounded-full cursor-pointer transition-colors z-20 ${index === 0 ? 'bg-white' : 'bg-white/30'} hover:bg-primary" aria-label="Anar a la imatge ${index + 1}"></button>
    `).join('') : '';
    ...
    const carouselNavHTML = images.length > 1 ? `
        <div class="absolute inset-y-0 left-0 flex items-center px-4 opacity-0 group-hover:opacity-100 transition-opacity z-20">
            <button onclick="window.prevSlide(${images.length})" class="size-12 rounded-full bg-black/40 text-white flex items-center justify-center hover:bg-primary backdrop-blur-sm transition-colors cursor-pointer" aria-label="Imatge anterior del carrusel">
        ...
        <div class="absolute inset-y-0 right-0 flex items-center px-4 opacity-0 group-hover:opacity-100 transition-opacity z-20">
            <button onclick="window.nextSlide(${images.length})" class="size-12 rounded-full bg-black/40 text-white flex items-center justify-center hover:bg-primary backdrop-blur-sm transition-colors cursor-pointer" aria-label="Imatge següent del carrusel">
    ...
    // CORRECCIÓN SEMÁNTICA: Se eliminó la etiqueta envolvente externa <main> (ya inyectada por app.js) y se pasó a <div class="grid"> estructurado
    return `
    <div class="w-full">
        <div class="max-w-7xl mx-auto px-4 py-4 sm:px-6 lg:px-8 flex items-center justify-between">
            <nav class="flex items-center gap-2 text-sm font-medium text-slate-500" aria-label="Fil d'ariadna">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
                        ...
                        <div class="bg-surface">
                            <div class="flex items-center justify-between mb-4">
                                ...
                                <button id="tts-toggle-btn" class="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-surface border border-white/10 hover:border-primary text-xs font-semibold text-slate-300 hover:text-white transition-all cursor-pointer">
                                    <span id="tts-icon" class="material-icons text-sm">volume_up</span>
            ...
            const audioObj = subjectOfArr.find(s => s['@type'] === 'AudioObject');
            if (audioObj) {
                return `
                                <div id="audio-player-container" class="bg-surface border border-white/10 rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between shadow-lg gap-6">
                                    ...
                                    <div class="flex items-center gap-5 w-full md:w-auto">
                                        <div class="audio-icon-container size-14 rounded-full bg-primary flex items-center justify-center shadow-[0_0_20px_rgba(48,137,48,0.4)] transition-all duration-300">
                                            <span class="material-icons text-white text-3xl">graphic_eq</span>
                                        </div>
                                        <div>
                                    ...
                                    <div class="flex items-center justify-center gap-4 flex-1 w-full md:w-auto mr-0 md:ml-4">
                                        <button class="play-btn size-12 rounded-full border border-forest-neutral-700 flex items-center justify-center hover:bg-primary transition-colors text-slate-100 shrink-0" aria-label="Reproduir àudio">
                                        ...
                                        <div class="audio-volume hidden sm:flex items-center gap-2 ml-2">
                                            <span class="material-icons text-forest-neutral-400 text-sm">volume_up</span>
                                            <input type="range" class="audio-volume-slider volume-slider" min="0" max="1" step="0.05" value="1" aria-label="Control de volum de l'àudio">
                                        </div>
                                    </div>
                                </div>`;
            } else {
                                ...
                                <div class="bg-surface border border-white/10 rounded-2xl p-8 flex items-center justify-between shadow-lg opacity-50">
                                    <div class="flex items-center gap-5">
                                        <div class="size-14 rounded-full bg-forest-neutral-800 flex items-center justify-center">
                                            <span class="material-icons text-forest-neutral-500 text-3xl">mic_off</span>
                                        </div>
                                        <div>
                ...
                videoHtml = `
                                        <figure id="video-wrapper" class="video-container relative overflow-hidden rounded-2xl aspect-video bg-black ring-1 ring-white/10 shadow-2xl">
                                            ...
                                            <div id="error-overlay" class="overlay hidden absolute inset-0 flex flex-col items-center justify-center bg-black/60 z-20">
                                                ...
                                                <p id="error-message" class="text-white">Ha ocurrent un error reproduint el vídeo.</p>
                                            </div>
                                            ...
                                            <div id="video-controls" class="controls-container" data-state="hidden" style="display: none;">
                                                <div class="progress-wrapper" id="progress-wrapper">
                                                    <input type="range" id="progress-bar" class="progress-bar" min="0" max="100" value="0" step="0.1" aria-label="Barra de progrés del vídeo">
                                                    <div class="progress-filled" id="progress-filled"></div>
                                                </div>
                                                <div class="controls-main">
                                                    <div class="controls-left">
                                                        <button id="play-pause-btn" class="control-btn" title="Reproducir/Pausar" aria-label="Reproduir o pausar vídeo">
                                                            <span class="material-icons">play_arrow</span>
                                                        </button>
                                                        <div class="volume-container hidden sm:flex">
                                                            <button id="mute-btn" class="control-btn" title="Silenciar" aria-label="Silenciar volum del vídeo">
                                                                <span class="material-icons">volume_up</span>
                                                            </button>
                                                            <input type="range" id="volume-slider" class="volume-slider" min="0" max="1" step="0.05" value="0.5" aria-label="Control de volum del vídeo">
                                                        </div>
                                                    ...
                                                    <div class="controls-right">
                                                        <button id="fullscreen-btn" class="control-btn" title="Pantalla completa" aria-label="Visualitzar el vídeo a pantalla completa">
            ...
            const linksHtml = links.map(link => {
                ...
                return `
                                        <a class="flex items-center justify-between p-4 rounded-xl bg-background-dark border border-white/10 hover:border-primary transition-all group" href="${url}" target="_blank" rel="noopener noreferrer">
                                            ...
                                            <span class="material-icons text-sm text-forest-neutral-500 group-hover:text-primary transition-colors">${actionIcon}</span>
                                        </a>`;
            }).join('');
            ...
            if (isFav) {
                return `
                            <button onclick="window.toggleFav(event, '${plantId}'); window.navigateSPA('plant-detail', '${plantId}');" class="w-full py-4 px-6 bg-red-500/20 hover:bg-red-500 text-red-500 hover:text-white font-bold rounded-xl border border-red-500/30 flex items-center justify-center gap-3 transition-all group">
                                <span class="material-icons transition-all">heart_broken</span>
                                Llevar de Preferits
                            </button>
                            `;
            } else {
                return `
                            <button onclick="window.toggleFav(event, '${plantId}'); window.navigateSPA('plant-detail', '${plantId}');" class="w-full py-4 px-6 bg-primary hover:bg-primary-dark text-white font-bold rounded-xl shadow-lg flex items-center justify-center gap-3 transition-all group active:scale-[0.98]">
                                <span class="material-icons transition-all">favorite</span>
                                Afegir a Preferits
                            </button>
                            `;
            }
                    ...
                    <div class="bg-surface rounded-xl border border-white/10 overflow-hidden shadow-xl">
